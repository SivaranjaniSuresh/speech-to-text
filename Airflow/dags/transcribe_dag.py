...
dag = DAG(
    ...
    description="Transcribe audio, upload transcript, and move audio file",
...
def transcribe_and_upload_s3_task(**kwargs):
    selected_file = kwargs["ti"].xcom_pull(key="selected_file")
    # Download the audio file from the S3 URL
...
def answer_default_questions(**kwargs):
    transcript_data = kwargs["ti"].xcom_pull(key="transcript_data")
    selected_file = kwargs["ti"].xcom_pull(key="selected_file")
...
transcribe_and_upload_s3 = PythonOperator(
    task_id="transcribe_and_upload_s3",
    python_callable=transcribe_and_upload_s3_task,
    provide_context=True,
    dag=dag,
)

push_s3_link = PythonOperator(
    task_id="push_s3_link_to_xcom",
    python_callable=push_s3_link_to_xcom,
    provide_context=True,
    dag=dag,
)

answer_default_questions = PythonOperator(
    task_id="answer_default_questions",
    python_callable=answer_default_questions,
    provide_context=True,
    dag=dag,
)

push_s3_link >> transcribe_and_upload_s3 >> answer_default_questions
